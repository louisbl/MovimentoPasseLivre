...
*/
	...
	static public function drawPlane(graphics:Graphics,
	                                 bitmap:BitmapData,
	                                 p1:Point,
	                                 p2:Point,
	                                 p3:Point,
		...
		var pc:Point = getIntersection(p1, p3, p2, p4); // Central point
		...
		var ll2:Float = Point.distance(pc, p3);

		// Lengths of second diagonal
		var lr1:Float = Point.distance(p2, pc);
		var lr2:Float = Point.distance(pc, p4);

		// Ratio between diagonals
		var f:Float = (ll1 + ll2) / (lr1 + lr2);

		data.vertices	= [p1.x, p1.y, p2.x, p2.y, p4.x, p4.y, p3.x, p3.y];
		data.indices 	= [0,1,2, 1,3,2];
		data.uvtData 	= [0,0,(1/ll2)*f, 1,0,(1/lr2), 0,1,(1/lr1), 1,1,(1/ll1)*f];
		...
		#if flash
			Vector.ofArray( data.vertices ),
			Vector.ofArray( data.indices ),
			Vector.ofArray( data.uvtData )
		#else
			data.vertices,
			data.indices,
			data.uvtData
